...
public class view_notes extends Activity {
	...
	String URL_ChapDownload = ip+"Note_Download.php";
//	String URL_SHEDULE = "http://192.168.1.11:8084/ServerCOde/Shedule.php";
	String URL_DOWNLOAD;
	ArrayList<time_tbl> TopList = new ArrayList<time_tbl>();
	ListView lv;
...
String id,file;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sp = PreferenceManager.getDefaultSharedPreferences(view_notes.this);
		myedit = sp.edit();
	...
	public void downloadFile() {
		...
		request.setDestinationInExternalFilesDir(view_notes.this, null,
				"" + file);
...
//
	...
	private class GetChaps extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(view_notes.this);
			pDialog.setMessage("Wait..");
			pDialog.setCancelable(false);
			pDialog.show();
				...
				try {
					...
					if (jsonobj != null) {
						...
						for (int i = 0; i < pre.length(); i++) {
							...
							Log.e("jsonobjct", ">" + job);
						time_tbl c = new time_tbl(job.getString("id"),
								
								job.getString("file"),job.getString("sub"),job.getString("top"));
		...
		protected void onPostExecute(Void result) {
			...
			ChapterAdapter ta = new ChapterAdapter(view_notes.this, TopList);
			lv.setAdapter(ta);
